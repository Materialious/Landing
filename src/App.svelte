...
<main class="responsive max">
  <article class="transparent header">
    ...
    {#if !release.draft}
      <button class="extra" style="margin-bottom: 1em;" onclick={showApps}>
        <i>download</i>
        <span>Download</span>
      </button>
    {/if}
  ...
  {#if shownSection === "apps"}
    ...
    <div>
      ...
      {#each Object.keys(assetsPlatforms) as platform}
        <div class="page padding" class:active={activePlatform === platform}>
          <ul>
            {#each assetsPlatforms[platform] as asset}
              <li>
                ...
                >
                  <div class={getClass(asset.asset.name)}></div>
                  <span>
                    {asset.extracted.ext}
